...
public:
    ...
    {
        return (addr % get_program_size() == 0 && size % get_program_size() == 0
                && addr + size <= this->size());
    }
    ...
    {
        return (addr % get_erase_size() == 0 && size % get_erase_size() == 0
                && addr + size <= this->size());
    }
...
inline void IBlockDevice::lock()
{
}

inline void IBlockDevice::unlock()
{
}
